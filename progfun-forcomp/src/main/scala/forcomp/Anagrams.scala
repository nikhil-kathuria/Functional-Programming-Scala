...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    if (w.isEmpty) List()
     ...
     val wordmap = w.toLowerCase.filter(_.isLetter) groupBy(c=> c)
     (for ((word, occurrence) <- wordmap) yield word -> occurrence.length).toList.sorted
    }
  
  /** Converts a sentence into its character occurrence list. */
  
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val totalstring = (s foldLeft "") (_++_)
    wordOccurrences(totalstring)
  }
...
**/
   ...
   */
   ...
   def combinations(occurrences: Occurrences): List[Occurrences] = 
    occurrences match {
      case Nil => List(List())
      case head :: tail =>   
        val totalcombs = combinations(tail)
        totalcombs:::( for{
          valcombs <- totalcombs
          changecount <- 1 to head._2 
        } yield (head._1,changecount)::valcombs)
   }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val totallist = x:::y
    for{  
      totalpairs <- totallist
      totalnot <- totallist if((totalpairs._1 == totalnot._1) || (totalpairs._2-totalnot._2>0))
    }yield (totalnot._1,(totalpairs._2-totalnot._2))
  }
